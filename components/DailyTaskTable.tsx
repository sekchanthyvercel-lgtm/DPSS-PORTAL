...
export const DailyTaskTable: React.FC<DailyTaskTableProps> = ({
  ...
  const getLevelColor = (level?: string) => {
    ...
    if (l.includes('1A')) return 'bg-purple-600 text-white';
    if (l.includes('1B')) return 'bg-orange-500 text-white';
    if (l.includes('2A')) return 'bg-amber-500 text-white';
    if (l.includes('2B')) return 'bg-blue-600 text-white';
    if (l.includes('3A')) return 'bg-indigo-600 text-white';
    if (l.includes('3B')) return 'bg-violet-600 text-white';
    if (l.includes('4A')) return 'bg-sky-600 text-white';
        ...
        <div className="flex items-center gap-2">
          <div className="relative w-40">
            ...
            <input 
              type="text" 
              placeholder="Search..." 
              className="w-full h-9 pl-9 pr-3 bg-white/[0.05] border border-white/10 rounded-xl text-[10px] font-black text-slate-900 uppercase outline-none focus:ring-4 focus:ring-orange-500/10 transition-all placeholder:text-slate-500 backdrop-blur-[4px]"
          ...
          {role === 'Admin' && (
            <button 
              ...
              className="w-9 h-9 bg-red-50 text-red-400 border border-red-50 rounded-xl flex items-center justify-center hover:bg-red-500 hover:text-white transition-all shadow-sm"
            ...
            <tbody className="divide-y divide-white/5">
              {filteredStudents.length === 0 ? (
                ...
                <tr key={s.id} className={`transition-colors group h-12 ${getRowBg(idx)} hover:brightness-95`}>
                  <td className="text-center text-[9px] font-black text-slate-300 border-r border-slate-100 sticky left-0 z-30 bg-inherit">{idx + 1}</td>
                  <td className={`px-4 border-r border-slate-100 sticky left-10 z-30 group-hover:opacity-90 transition-opacity ${getLevelColor(s.level)} bg-inherit`}>
                    <input 
                      value={s.name} 
                      onChange={e => updateField(s.id, 'name', e.target.value)}
                      placeholder="Task Name"
                      className="w-full bg-transparent text-[11px] font-black outline-none placeholder:text-white/30"
                    />
                  </td>
                  <td className={`border-r border-slate-100 ${getLevelColor(s.level)}`}>
                    <input 
                      value={s.level || ''} 
                      onChange={e => updateField(s.id, 'level', e.target.value)}
                      placeholder="Level"
                      className="w-full h-full px-2 bg-transparent text-[10px] font-black outline-none text-center placeholder:text-white/30"
                    />
                  </td>
